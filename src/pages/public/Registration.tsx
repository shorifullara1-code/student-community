...
import { User, Phone, Mail, Home, Building, CheckCircle, AlertCircle } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message?.includes('does not exist')) {
        setErrorMsg('Supabase-এ "registrations" টেবিল পাওয়া যায়নি। দয়া করে এডমিনকে একটি "registrations" টেবিল তৈরি করতে বলুন। (অথবা নিচের SQL কোডটি ব্যবহার করুন)');
      } else if (err.message?.includes('VITE_SUPABASE_ANON_KEY') || err.message?.includes('JWT') || err.message?.includes('Invalid API key')) {
        setErrorMsg('Supabase কনফিগারেশন ভুল বা Invalid API key। Project variables ঠিক আছে কিনা চেক করুন। টেবিল তৈরি না থাকলে নিচের SQL কোডটি ব্যবহার করতে পারেন।');
      } else {
    ...
    <div className="max-w-2xl mx-auto bg-white border border-gray-200 rounded-lg shadow-sm p-6 md:p-8">
      <div className="border-b border-gray-200 pb-4 mb-6">
        ...
        <p className="text-gray-600 mt-2 text-sm">
          সাভার স্টুডেন্ট কমিউনিটির সদস্য হতে নিচের ফর্মটি পূরণ করুন। আপনার তথ্যগুলো এডমিন প্যানেলে সংরক্ষিত থাকবে।
        </p>
      </div>
      ...
      {success && (
        <div className="bg-green-50 border border-green-200 text-green-700 p-4 rounded-lg mb-6 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm mt-1">আপনার তথ্য আমাদের ডেটাবেসে সংরক্ষিত হয়েছে। প্রয়োজনে আমারা আপনার সাথে যোগাযোগ করব।</p>
          </div>
        </div>
      )}
      ...
      {errorMsg && (
        <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg mb-6 flex flex-col gap-3">
          ...
          {(errorMsg.includes('SQL') || errorMsg.includes('Invalid API key') || errorMsg.includes('টেবিল')) && (
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">ইমেইল (যদি থাকে)</label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
           <div>
             <label className="block text-sm font-semibold text-gray-700 mb-1">নিজ এলাকা (সাভার) *</label>
             <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
